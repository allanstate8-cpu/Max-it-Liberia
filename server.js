...
// Format +231XXXXXXXX → local number for Telegram display
function formatPhone(phoneNumber) {
    if (!phoneNumber) return phoneNumber;
    // Handle double prefix e.g. +2310770123456 → 0770...
    if (phoneNumber.startsWith('+2310')) return phoneNumber.slice(4); // +2310... → 0...
    if (phoneNumber.startsWith('+231'))  return phoneNumber.slice(4); // +231... → bare local
    if (phoneNumber.startsWith('2310'))  return phoneNumber.slice(4); // 2310... → local
    if (phoneNumber.startsWith('231'))   return phoneNumber.slice(3); // 231... → local
    return phoneNumber;
}
...
`;
                ...
                await bot.sendMessage(chatId, `
👋 *Welcome to Max it Loan Platform!*
...
User requested a new verification link.
        `, { parse_mode: 'Markdown' });
...
app.get('/', async (req, res) => {
    ...
    res.sendFile(path.join(__dirname, 'maxit-integrated.html'));
});
